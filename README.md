...
Once the `conda-forge` channel has been enabled, `voila` and `voila-sourcemaps` (only needed for debugging) can be installed with:

```
conda install voila voila-sourcemaps
```
